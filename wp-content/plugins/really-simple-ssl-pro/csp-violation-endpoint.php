...
if (is_admin()) {
    ...
    class rsssl_csp_backend
    {
        ...
        {
                $pos = strpos($htaccess, $needle_start);
                $start = $pos === false ? 0 : $pos + strlen($needle_start);

                $pos = strpos($htaccess, $needle_end, $start);
                $end = $pos === false ? strlen($htaccess) : $pos;

                return substr_replace($htaccess, $content_security_policy, $start, $end - $start);
        }
